...
class Ha(object):
    ...
    def process_sync_replication(self):
        ...
        QuorumStateResolver that given a current state and set of desired synchronous nodes and replication level
        outputs changes to DCS and synchronous replication in correct order to reach the desired state.
        ...
        if self.is_synchronous_mode():
            ...
            # So preserving old logic
            if not self.state_handler.use_multiple_sync:
                current = self.cluster.sync.leader and list(self.cluster.sync.members)[0]
                ...
                if picked != current:
                    ...
                    if current:
                        ...
                        if not self.dcs.write_sync_state(self.state_handler.name, 0, {}, index=self.cluster.sync.index):
                    ...
                    self.state_handler.set_synchronous_state(1, {picked})
                ...
                self.process_quorum_sync_replication()
        else:
            self.disable_synchronous_replication()

    def process_quorum_sync_replication(self):
    ...
    def process_sync_replication_prepromote(self):
        ...
        DCS state to ourselves and add replicas as they connect.
        """
        ...
        # Node outside voters achieved quorum and got leader
        if self.state_handler.name not in sync:
            numsync += 1
            sync = sync.union([self.state_handler.name])
        ...
        if not self.state_handler.use_multiple_sync and numsync > 2:
            ...
            self.state_handler.set_synchronous_state(numsync, {self.state_handler.name})
    ...
    def enforce_master_role(self, message, promote_message):
        ...
        if self.state_handler.is_leader():
            ...
            self.state_handler.set_synchronous_state(1 if self.is_synchronous_mode_strict() else None,
                                                     {} if self.is_synchronous_mode_strict() else None)
    ...
    def _is_healthiest_node(self, members, quorum=0, voting_set=frozenset(), check_replication_lag=True):
    ...
    def is_healthiest_node(self):
        ...
        if self.is_synchronous_mode() and self.cluster.sync.leader:
            ...
            voting_set = self.cluster.sync.members
        else:
            quorum = 0
            voting_set = set()

        return self._is_healthiest_node(members.values(), quorum, voting_set)
    ...
    def restart_matches(self, role, postgres_version, pending_restart):
        ...
        if (postgres_version and
                    self.state_handler.postgres_version_to_int(postgres_version) <= int(
