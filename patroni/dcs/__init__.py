...
class SyncState(namedtuple('SyncState', 'index,leader,quorum,members')):
    ...
    def from_node(index, value):
        ...
        >>> SyncState.from_node(1, {"leader": "leader"}).leader == "leader"
        True
        >>> SyncState.from_node(1, {"leader": "a", "sync_standby": "b"}).members == set("b")
        True
        >>> SyncState.from_node(1, {"leader": "a", "members": "b, c", "quorum": 2}).members == set("bc")
        True
        """
        try:
            if isinstance(value, dict):
            ...
            if 'members' in data:
                ...
                quorum = 1
                members = frozenset(str(m) for m in [data.get('sync_standby')] if m)
