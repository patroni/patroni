...
from test_postgresql import psycopg2_connect, MockPostmaster, MockCursor
...
def get_cluster_initialized_without_leader(leader=False, failover=None, sync=None):
    ...
    if sync:
        syncstate = SyncState(0, sync[0], sync[1], frozenset(sync[1]))
    else:
...
class TestHa(unittest.TestCase):
    ...
    def test_process_sync_replication(self):
        ...
        self.ha.cluster = get_cluster_initialized_with_leader(sync=(1,('leader', 'other')))
        with patch.object(MockCursor, 'fetchone', Mock(return_value=('', '', ''))):
            self.ha.run_cycle()
        ...
        self.ha.run_cycle()
        mock_set_sync.assert_called_once_with(2, {'leader', 'other2'})
