...
class MockRestApiServer(RestApiServer):
    def __init__(self, Handler, request, config=None):
        ...
        config = config or {
            ...
            "certfile": "dumb",
            "verify_client": "a",
            "http_extra_headers": {"foo": "bar"},
            "https_extra_headers": {"foo": "sbar"},
        }
...
class TestRestApiHandler(unittest.TestCase):
    ...
    def test_do_GET(self):
        ...
        with patch.object(
            GlobalConfig, "is_synchronous_mode", PropertyMock(return_value=True)
        ):
        ...
        with patch.object(
            RestApiHandler, "get_postgresql_status", Mock(return_value={})
        ):
            MockRestApiServer(RestApiHandler, "GET /replica")
        with patch.object(
            RestApiHandler,
            "get_postgresql_status",
            Mock(return_value={"role": "primary"}),
        ):
            MockRestApiServer(RestApiHandler, "GET /replica")
        with patch.object(
            RestApiHandler,
        ...
        with patch.object(MockHa, "is_leader", Mock(return_value=True)):
            ...
            MockRestApiServer(RestApiHandler, "GET /read-only-sync")
            with patch.object(
                GlobalConfig, "is_standby_cluster", Mock(return_value=True)
            ):
        ...
        with patch.object(
            RestApiServer,
            "query",
            Mock(return_value=[("", 1, "", "", "", "", False, None, None, "")]),
        ):
        ...
        with patch.object(
            RestApiHandler,
            "get_postgresql_status",
            Mock(return_value={"role": "standby_leader"}),
        ):
            MockRestApiServer(
                RestApiHandler,
                "GET /standby_leader?lag=1M&"
                "tag_key1=true&tag_key2=false&"
                "tag_key3=1&tag_key4=1.4&tag_key5=RandomTag",
            )
            MockRestApiServer(
    ...
    def test_do_GET_liveness(self, mock_dcs):
        ...
        self.assertIsNotNone(
            MockRestApiServer(RestApiHandler, "GET /liveness HTTP/1.0")
        )
    ...
    def test_do_GET_readiness(self):
        ...
        with patch.object(
            MockPostgresql, "state", PropertyMock(return_value="stopped")
        ):
            self.assertIsNotNone(
                MockRestApiServer(RestApiHandler, "GET /readiness HTTP/1.0")
            )
    ...
    def test_do_GET_failsafe(self, mock_dcs):
        type(mock_dcs).failsafe = PropertyMock(
            return_value={"node1": "http://foo:8080/patroni"}
        )
    ...
    def test_do_POST_failsafe(self):
        with patch.object(
            MockHa, "is_failsafe_mode", Mock(return_value=False), create=True
        ):
            self.assertIsNotNone(
                MockRestApiServer(
                    RestApiHandler, "POST /failsafe HTTP/1.0" + self._authorization
                )
            )
        with patch.object(
            MockHa, "is_failsafe_mode", Mock(return_value=True), create=True
        ):
            self.assertIsNotNone(
                MockRestApiServer(
                    RestApiHandler,
                    "POST /failsafe HTTP/1.0"
                    + self._authorization
                    + '\nContent-Length: 9\n\n{"a":"b"}',
                )
            )
    ...
    def test_do_POST_reload(self):
        self.assertIsNotNone(
            MockRestApiServer(
                RestApiHandler, "POST /reload HTTP/1.0" + self._authorization
            )
        )
    ...
    def test_do_POST_sigterm(self):
        self.assertIsNotNone(
            MockRestApiServer(
                RestApiHandler, "POST /sigterm HTTP/1.0" + self._authorization
            )
        )
    ...
    def test_do_POST_restart(self):
        ...
        for retval in (True, False):
            with patch.object(
                MockHa, "schedule_future_restart", Mock(return_value=retval)
            ):
        ...
        with patch.object(GlobalConfig, "is_paused", PropertyMock(return_value=True)):
            MockRestApiServer(
                RestApiHandler,
                make_request(schedule="2016-08-42 12:45TZ+1", role="primary"),
            )
            # Valid timeout
    ...
    def test_do_DELETE_restart(self):
        for retval in (True, False):
            with patch.object(
                MockHa, "delete_future_restart", Mock(return_value=retval)
            ):
    ...
    def test_RestApiServer_query(self):
        with patch.object(
            MockCursor, "execute", Mock(side_effect=psycopg.OperationalError)
        ):
            self.assertIsNotNone(MockRestApiServer(RestApiHandler, "GET /patroni"))
        with patch.object(
            MockPostgresql, "connection", Mock(side_effect=psycopg.OperationalError)
        ):
            self.assertIsNotNone(MockRestApiServer(RestApiHandler, "GET /patroni"))
    ...
    def test_do_POST_switchover(self, dcs):
        ...
        MockRestApiServer(RestApiHandler, request)
        with patch.object(
            GlobalConfig, "is_paused", PropertyMock(return_value=True)
        ...
        MockRestApiServer(
            RestApiHandler, request + '1016-02-15T18:13:30.568224+01:00"}'
        )

        # Invalid date
        self.assertIsNotNone(
            MockRestApiServer(
                RestApiHandler, request + '2010-02-29T18:13:30.568224+01:00"}'
            )
        )
...
class TestRestApiServer(unittest.TestCase):
    ...
    def test_reload_config(self):
        ...
        with patch.object(
            socket.socket, "setsockopt", Mock(side_effect=socket.error)
    ...
    def test_socket_error(self):
        self.assertRaises(
            socket.error, MockRestApiServer, Mock(), "", {"listen": "*:8008"}
        )
