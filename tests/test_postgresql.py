...
class TestPostgresql(BaseTestPostgresql):
    ...
    def test_pick_sync_standby(self):
        ...
        with patch.object(Postgresql, "query", side_effect=[
                    mock_cursor,
                    [(self.leadermem.name, 'streaming', 'sync', 1),
                     (self.me.name, 'streaming', 'async', 2),
                     (self.other.name, 'streaming', 'async', 2)]
                ]):
            self.assertEqual(self.p.pick_synchronous_standby(cluster), ([self.leadermem.name], [self.leadermem.name]))

        with patch.object(Postgresql, "query", side_effect=[
                    mock_cursor,
                    [(self.leadermem.name, 'streaming', 'potential', 1),
                     (self.me.name, 'streaming', 'async', 2),
                     (self.other.name, 'streaming', 'async', 2)]
                ]):
        ...
        with patch.object(Postgresql, "query", side_effect=[
                    mock_cursor,
                    [(self.me.name, 'streaming', 'async', 1),
                     (self.other.name, 'streaming', 'async', 2)]
                ]):
        ...
        with patch.object(Postgresql, "query", side_effect=[
                    mock_cursor,
                    [('missing', 'streaming', 'sync', 1),
                     (self.me.name, 'streaming', 'async', 2),
                     (self.other.name, 'streaming', 'async', 3)]
                ]):
            self.assertEqual(self.p.pick_synchronous_standby(cluster), ([self.me.name], []))
