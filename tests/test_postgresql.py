...
class TestPostgresql(BaseTestPostgresql):
    ...
    def test_has_role_based_config(self):
        """Test has_role_based_config property detects role-specific config keys."""
        # No role-based config
        self.assertFalse(self.p.config.has_role_based_config)

        # Add parameters_primary
        self.p.config._config['parameters_primary'] = {'work_mem': '128MB'}
        self.assertTrue(self.p.config.has_role_based_config)

        # Reset and test pg_hba_replica
        self.p.config._config.pop('parameters_primary')
        self.assertFalse(self.p.config.has_role_based_config)
        self.p.config._config['pg_hba_replica'] = ['host all all 0.0.0.0/0 md5']
        self.assertTrue(self.p.config.has_role_based_config)

        # Reset and test pg_ident_standby_leader
        self.p.config._config.pop('pg_hba_replica')
        self.assertFalse(self.p.config.has_role_based_config)
        self.p.config._config['pg_ident_standby_leader'] = ['map1 user1 postgres']
        self.assertTrue(self.p.config.has_role_based_config)

    def test_get_pg_hba_with_role_overrides(self):
        """Test _get_pg_hba returns role-specific config when available."""
        base_hba = ['host all all 0.0.0.0/0 md5']
        primary_hba = ['host all all 10.0.0.0/8 trust']

        config = {
            'pg_hba': base_hba,
            'pg_hba_primary': primary_hba
        }

        # When role is primary, should return primary-specific config
        self.p.set_role(PostgresqlRole.PRIMARY)
        result = self.p.config._get_pg_hba(config)
        self.assertEqual(result, primary_hba)

        # When role is replica, should return base config
        self.p.set_role(PostgresqlRole.REPLICA)
        result = self.p.config._get_pg_hba(config)
        self.assertEqual(result, base_hba)

        # When no pg_hba in config, should return None
        result = self.p.config._get_pg_hba({})
        self.assertIsNone(result)

    def test_get_pg_ident_with_role_overrides(self):
        """Test _get_pg_ident returns role-specific config when available."""
        base_ident = ['map1 user1 postgres']
        replica_ident = ['map2 user2 postgres']

        config = {
            'pg_ident': base_ident,
            'pg_ident_replica': replica_ident
        }

        # When role is replica, should return replica-specific config
        self.p.set_role(PostgresqlRole.REPLICA)
        result = self.p.config._get_pg_ident(config)
        self.assertEqual(result, replica_ident)

        # When role is primary, should return base config
        self.p.set_role(PostgresqlRole.PRIMARY)
        result = self.p.config._get_pg_ident(config)
        self.assertEqual(result, base_ident)

        # When no pg_ident in config, should return None
        result = self.p.config._get_pg_ident({})
        self.assertIsNone(result)

    def test_get_server_parameters_with_role_overrides(self):
        """Test get_server_parameters applies role-specific parameter overrides."""
        config = {
            'parameters': {
                'work_mem': '64MB',
                'max_connections': '100',
            ...
            'parameters_primary': {'work_mem': '128MB'},
            'listen': '0.0.0.0:5432'
        }

        # When role is primary, work_mem should be overridden
        self.p.set_role(PostgresqlRole.PRIMARY)
        params = self.p.config.get_server_parameters(config)
        self.assertEqual(params['work_mem'], '128MB')
        self.assertEqual(params['max_connections'], '100')  # Not overridden

        # When role is replica, should use base config
        self.p.set_role(PostgresqlRole.REPLICA)
        params = self.p.config.get_server_parameters(config)
        self.assertEqual(params['work_mem'], '64MB')

    @patch.object(ConfigHandler, 'replace_pg_ident')
    @patch.object(ConfigHandler, 'replace_pg_hba')
    @patch.object(ConfigHandler, 'write_postgresql_conf')
    @patch.object(Postgresql, 'role', new_callable=PropertyMock, return_value=PostgresqlRole.PRIMARY)
    def test_write_all_config_files_with_default_role(self, mock_role, mock_write_conf,
                                                      mock_replace_hba, mock_replace_ident):
        """Test write_all_config_files writes all configs with default role."""
        self.p.config._config = {
            'parameters': {
                'work_mem': '64MB',
                'max_prepared_transactions': '0',
                'max_connections': '100'
            ...
            'pg_hba_primary': ['host all all 10.0.0.0/8 trust'],
            'pg_ident': ['map1 user1 postgres'],
            'listen': '0.0.0.0:5432'
        }

        self.p.config.write_all_config_files()

        # Verify all write methods were called with expected parameters
        mock_write_conf.assert_called_once_with(None, role=None)
        mock_replace_hba.assert_called_once_with(role=None)
        mock_replace_ident.assert_called_once_with(role=None)

    @patch.object(ConfigHandler, 'replace_pg_ident')
    @patch.object(ConfigHandler, 'replace_pg_hba')
    @patch.object(ConfigHandler, 'write_postgresql_conf')
    def test_write_all_config_files_with_role(self, mock_write_conf, mock_replace_hba, mock_replace_ident):
        """Test write_all_config_files always writes configs regardless of has_role_based_config."""
        self.p.config.write_all_config_files(role=PostgresqlRole.REPLICA)

        # Should write configs even when no role-based config exists
        mock_write_conf.assert_called_once_with(None, role=PostgresqlRole.REPLICA)
        mock_replace_hba.assert_called_once_with(role=PostgresqlRole.REPLICA)
        mock_replace_ident.assert_called_once_with(role=PostgresqlRole.REPLICA)

    @patch.object(Postgresql, 'reload')
    @patch.object(ConfigHandler, 'write_all_config_files')
    @patch.object(Postgresql, 'role', new_callable=PropertyMock, return_value=PostgresqlRole.PRIMARY)
    def test_apply_role_based_config_if_needed_with_overrides(self, mock_role, mock_write, mock_reload):
        """Test apply_role_based_config_if_needed applies config and reloads when overrides exist."""
        self.p.config._config['parameters_primary'] = {'work_mem': '128MB'}

        self.p.apply_role_based_config_if_needed()

        mock_write.assert_called_once_with(role=None)
        mock_reload.assert_called_once_with(block_callbacks=True)

    @patch.object(Postgresql, 'reload', return_value=False)
    @patch.object(ConfigHandler, 'write_all_config_files')
    def test_apply_role_based_config_if_needed_not_running(self, mock_write, mock_reload):
        """Test apply_role_based_config_if_needed works even when not running (reload will fail gracefully)."""
        self.p.config._config['parameters_primary'] = {'work_mem': '128MB'}
    ...
    @patch.object(ConfigHandler, 'write_all_config_files')
    def test_apply_role_based_config_if_needed_no_config(self, mock_write):
        """Test apply_role_based_config_if_needed returns early when no role-based config."""
        self.p.apply_role_based_config_if_needed()
        mock_write.assert_not_called()

    @patch.object(ConfigHandler, 'write_all_config_files', side_effect=Exception('test error'))
    @patch.object(Postgresql, 'role', new_callable=PropertyMock, return_value=PostgresqlRole.PRIMARY)
    def test_apply_role_based_config_if_needed_with_exception(self, mock_role, mock_write):
        """Test apply_role_based_config_if_needed propagates exceptions (no exception handling in method)."""
        self.p.config._config['parameters_primary'] = {'work_mem': '128MB'}

        # Should raise exception since we removed exception handling
        with self.assertRaises(Exception):
            self.p.apply_role_based_config_if_needed()

    @patch.object(ConfigHandler, 'write_all_config_files')
    @patch.object(PostmasterProcess, 'start')
    @patch.object(Postgresql, 'wait_for_port_open', return_value=True)
    @patch.object(Postgresql, 'is_running', return_value=False)
    def test_start_with_role_based_config(self, mock_is_running, mock_wait_for_port, mock_start, mock_write_all):
        """Test that start() applies role-based configuration when starting with a specific role."""
        # Set up role-based config
        self.p.config._config['parameters_replica'] = {'work_mem': '32MB'}
        self.p.config._config['pg_hba_replica'] = ['host all all 0.0.0.0/0 md5']

        mock_postmaster = MockPostmaster()
        mock_start.return_value = mock_postmaster

        # Start with replica role
        result = self.p.start(role=PostgresqlRole.REPLICA)
        self.assertTrue(result)

        # Verify write_all_config_files was called with configuration and the replica role
        mock_write_all.assert_called_once()
        call_kwargs = mock_write_all.call_args
        # First positional arg is configuration (effective_configuration)
        self.assertIsNotNone(call_kwargs[0][0])
        # Role kwarg should be replica
        self.assertEqual(call_kwargs[1]['role'], PostgresqlRole.REPLICA)

    @patch.object(ConfigHandler, 'build_recovery_params', return_value={'primary_conninfo': 'foo'})
    @patch.object(ConfigHandler, 'write_all_config_files')
    @patch.object(Postgresql, 'reload', return_value=True)
    @patch.object(Postgresql, 'is_running', Mock(return_value=True))
    def test_follow_with_role_change(self, mock_reload, mock_write_all, mock_build_recovery):
        """Test that follow() applies role-based configuration when role changes."""
        # Set up role-based config
        self.p.config._config['parameters_replica'] = {'work_mem': '32MB'}
        self.p.config._config['pg_hba_replica'] = ['host all all 0.0.0.0/0 md5']

        m = RemoteMember('leader', {'conn_url': 'postgres://foo', 'conn_kwargs': {'host': 'foo'}})

        # Follow with role change (simulates demotion from primary to replica)
        self.p.follow(m, role=PostgresqlRole.REPLICA, do_reload=True)

        # Verify write_all_config_files was called with the replica role
        mock_write_all.assert_called_with(role=PostgresqlRole.REPLICA)

    @patch.object(Postgresql, '_wait_promote', return_value=True)
    @patch.object(Postgresql, 'reload')
    @patch.object(ConfigHandler, 'write_all_config_files')
    @patch.object(Postgresql, 'pg_ctl', return_value=True)
    def test_promote_with_role_based_config(self, mock_pg_ctl, mock_write_all, mock_reload, mock_wait_promote):
        """Test that promote() applies role-based configuration after promotion."""
        self.p.config._config['parameters_primary'] = {'work_mem': '128MB'}
        self.p.set_role(PostgresqlRole.REPLICA)

        task = CriticalTask()
        result = self.p.promote(0, task)

        self.assertTrue(result)
        # Verify role-based config was applied after promotion with PRIMARY role
        mock_write_all.assert_called_once_with(role=PostgresqlRole.PRIMARY)
        mock_reload.assert_called_once_with(block_callbacks=True)
