Feature: permanent slots
  Scenario: check that physical permanent slots are created
    ...
    When I issue a PATCH request to http://127.0.0.1:8008/config with {"slots":{"test_physical":null,"postgres0":null,"postgres1":null},"postgresql":{"parameters":{"wal_level":"logical"}}}
    ...
    Then postgres0 has a physical replication slot named postgres1 after 10 seconds
    Then postgres0 has a physical replication slot named postgres2 after 10 seconds
  ...
  Scenario: check permanent physical slots that match with member names
    Given postgres1 has a physical replication slot named postgres0 after 2 seconds
