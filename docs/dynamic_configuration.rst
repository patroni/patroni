...
-  **metrics_collector_retention**: retention window (in seconds) used by Patroni's internal metrics collector to keep historical samples of recorded operation durations (for example, the HA loop cycle duration exposed through the ``/metrics`` endpoint as ``patroni_ha_loop_duration_avg`` and ``patroni_ha_loop_duration_p99``). Entries older than this value are discarded from the in-memory history. Default value: ``3600`` (one hour). Minimum allowed value: ``300`` (five minutes). The currently configured value for the node is also exposed as the ``patroni_metrics_collector_retention`` Prometheus metric on the ``/metrics`` endpoint.
