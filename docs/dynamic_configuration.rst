...
-  **synchronous\_cross\_site**: controls how Patroni selects synchronous standbys when the cluster spans multiple sites. Only takes effect when ``synchronous_mode`` is enabled and nodes have the ``site`` attribute configured. Possible values:

   -  ``any`` (default): no site-based filtering; any eligible replica may be selected as synchronous.
   -  ``local_only``: only replicas located in the same site as the primary are eligible. If no local replicas are available, ``synchronous_standby_names`` is set to an empty value (or to the strict-mode placeholder when combined with ``synchronous_mode_strict``).
   -  ``remote_only``: only replicas located in sites other than the primary's site are eligible. If no remote replicas are available, ``synchronous_standby_names`` is set to an empty value (or to the strict-mode placeholder when combined with ``synchronous_mode_strict``).
   -  ``prefer_local``: local replicas are preferred, but the selection is padded with remote replicas if there are not enough local replicas to satisfy ``synchronous_node_count``.
   -  ``prefer_remote``: remote replicas are preferred, but the selection is padded with local replicas if there are not enough remote replicas to satisfy ``synchronous_node_count``.
   -  ``balanced``: replicas are picked round-robin across sites, picking one replica from each remote site, then one from the local site, repeating until ``synchronous_node_count`` replicas are selected. Replicas without a configured site are considered last.

   When ``synchronous_cross_site`` is set to ``local_only`` or ``remote_only`` and no replicas matching the criterion are available while ``synchronous_mode_strict`` is enabled, Patroni clears the ``/sync`` DCS key and writes the ``__patroni_strict_sync_replica_placeholder__`` sentinel into ``synchronous_standby_names``, which blocks all writes until a qualifying replica becomes available again. See :ref:`cross-site synchronous mode <site_awareness_synchronous>` documentation for details and examples.
