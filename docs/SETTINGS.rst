...
-  **use\_unix\_socket**: specifies that Patroni should prefer to use unix sockets to connect to the cluster. Default value is ``true``. If ``unix_socket_directories`` is definded, Patroni will use first sutable value from it to connect to the cluster and fallback to tcp if nothing is sutable. If ``unix_socket_directories`` is not specified in ``postgresql.parameters``, Patroni will assume that default value should be used and omit ``host`` from connection parameters.
