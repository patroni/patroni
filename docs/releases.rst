...
- Add support for PostgreSQL 19 RC1 (Alexander Kukushkin)

  Extend GUC validator rules, properly handle the new auxiliary backends and the changed LSN format, and use ``CHECKPOINT (FLUSH_UNLOGGED)`` for internal operations.

- Support digest ACLs in Patroni ZooKeeper ``set_acls`` configuration (Igor Vasilkov)

  Previously, ``set_acls`` always applied ACLs with the ``x509`` scheme. Now, users can specify other supported ZooKeeper schemes, such as digest, directly in the key.
...
- Primary race backoff -- preventing false-positive failovers (Alexander Kukushkin)
  ...
  This feature allows postponing the leader race on standbys by ``primary_race_backoff`` seconds if WAL replication from the primary is still advancing. This minimizes unnecessary failovers caused by a briefly unresponsive Patroni.
...
- Add ``patroni_failover_priority`` Prometheus metric (Huseyin Demir)

  Expose the failover priority of a node via the ``/metrics`` endpoint.

- Make ``pysyncobj`` (Raft DCS) timeouts configurable (Marcelo Henrique Neppel)

  Introduce the ``raft.min_timeout``, ``raft.max_timeout``, ``raft.connection_timeout``, ``raft.append_entries_period``, ``raft.connection_retry_time``, and ``raft.leader_fallback_timeout`` configuration parameters to allow tuning of the Raft DCS timeouts for high-latency networks.
...
- Improve ``synchronous_mode_strict`` logic (Alexander Kukushkin)
  ...
  - If the ``/sync`` key has ``leader=node1``, ``sync_standby=node2,node3``, and both standby nodes suddenly stopped streaming, Patroni will set ``synchronous_standby_names`` to ``X (node2,node3)`` because these nodes are expected to have the latest commit.
  - If the ``/sync`` key has ``leader=node``, ``sync_standby=null``, and there is a manual failover to the asynchronous ``node3``, ``node3``, Patroni will set ``synchronous_standby_names`` to ``node1`` because the former primary is expected to have the latest commits.
  - If the ``/sync`` key is empty and strict synchronous mode is enabled, Patroni will set ``synchronous_standby_names`` to ``__patroni_strict_sync_replica_placeholder__``, meaning all commits will hang until an eligible node starts streaming from the primary.

- Drop failover slots with ``synced=false`` on standbys after a switchover (Alexander Kukushkin)

  Overcome the bug in the PostgreSQL failover slots implementation, where the ``slotsync`` worker refuses to synchronize such logical slots.

- Improve reliability of ``_is_postmaster_process()`` checks (Alexander Kukushkin)

  When parsing ``postmaster.pid`` to determine whether a PostgreSQL cluster is running for a given $PGDATA, Patroni previously relied solely on comparing the start time recorded in ``postmaster.pid`` with the process creation time reported by the OS. In some cases, the difference between these timestamps can exceed 3 seconds. In this scenario, Patroni incorrectly assumes PostgreSQL is not running and attempts to start a second instance. The improved check additionally verifies the process executable reported by the OS and the process' current working directory, which must match ``$PGDATA``.
...
- Improve switchover unknown-status messaging (Muhammad Umair Ali)
  ...
  Extend API timeout response with the timeout duration and report a ``result unknown`` status instead of ``failed`` in ``patronictl`` when the API returns a ``status unknown``.

- Improve logging handling of REST API connection resets (Kyle McLaren)

  Previously, Patroni's REST API logged a full Python stack trace as a warning every time a client reset the TCP connection before the response was fully written. This situation happens routinely whenever a load balancer or health-checker closes the connection right after reading the health-check response. Patroni now logs these events at the ``DEBUG`` level instead.
